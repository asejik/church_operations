...
import {
  Plus, Search, TrendingUp, TrendingDown, Wallet, ArrowUpRight, ArrowDownLeft,
  CheckCircle, XCircle, Clock, Loader2, MessageSquare, X, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export const FinancePage = () => {
  ...
  const [currentDate, setCurrentDate] = useState(new Date()); // State for Month View
  ...
  const filterByUnitAndDate = (item: any, dateField: string) => {
    ...
    // Check if item date is within the selected month
  ...
  // Stats (Unit Only - Based on filtered month)
  const totalIncome = filteredLedger.filter(t => t.type === 'income').reduce((a, c) => a + Number(c.amount), 0);
  const totalExpense = filteredLedger.filter(t => t.type === 'expense').reduce((a, c) => a + Number(c.amount), 0);
      ...
      <div className="flex flex-col md:flex-row gap-3 items-end md:items-center bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
         ...
         <div className="flex gap-2 w-full md:w-auto overflow-x-auto items-center">
            ...
            {!isAdmin && profile?.role !== 'unit_pastor' && (
               <Button size="sm" className="h-10 ml-2" onClick={() => activeTab === 'ledger' ? setIsLedgerModalOpen(true) : setIsRequestModalOpen(true)}>
            ...
            {loading ? <div className="p-8 text-center text-slate-400">Loading...</div> : (
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredRequests.length === 0 ? ( <tr><td colSpan={isAdmin ? 7 : 6} className="px-6 py-8 text-center text-slate-500">No requests found for {format(currentDate, 'MMMM')}.</td></tr> ) : ( filteredRequests.map((req, index) => (
      ...
      {activeTab === 'ledger' && !isAdmin && (
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm"><div className="flex items-center gap-2 text-slate-500 mb-1"><Wallet className="h-4 w-4" /><span className="text-xs font-bold uppercase">Monthly Balance</span></div><p className={`text-2xl font-bold ${balance >= 0 ? 'text-slate-900' : 'text-red-600'}`}>{formatCurrency(balance)}</p></div>
            <div className="rounded-xl border border-green-100 bg-green-50/50 p-4 shadow-sm"><div className="flex items-center gap-2 text-green-600 mb-1"><TrendingUp className="h-4 w-4" /><span className="text-xs font-bold uppercase">{format(currentDate, 'MMM')} Income</span></div><p className="text-2xl font-bold text-green-700">{formatCurrency(totalIncome)}</p></div>
            <div className="rounded-xl border border-red-100 bg-red-50/50 p-4 shadow-sm"><div className="flex items-center gap-2 text-red-600 mb-1"><TrendingDown className="h-4 w-4" /><span className="text-xs font-bold uppercase">{format(currentDate, 'MMM')} Expense</span></div><p className="text-2xl font-bold text-red-700">{formatCurrency(totalExpense)}</p></div>
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm mt-4">
            {loading ? <div className="p-8 text-center text-slate-400">Loading...</div> : (
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredLedger.length === 0 ? ( <tr><td colSpan={6} className="px-6 py-8 text-center text-slate-500">No records found for {format(currentDate, 'MMMM')}.</td></tr> ) : ( filteredLedger.map((t, index) => (
