...
import { db, type FinanceItem, type BudgetRequest } from '@/lib/db';
import { supabase } from '@/lib/supabase';
import { Button } from '@/components/ui/Button';
import { Modal } from '@/components/ui/Modal';
import { Plus, Search, TrendingUp, TrendingDown, Wallet, ArrowUpRight, ArrowDownLeft, Send, CheckCircle, XCircle, Clock } from 'lucide-react';
import { toast } from 'sonner';
import { useProfile } from '@/hooks/useProfile';

// --- MODAL: ADD LEDGER TRANSACTION (SYNCED) ---
const AddLedgerModal = ({ isOpen, onClose, onComplete }: { isOpen: boolean; onClose: () => void; onComplete: () => void }) => {
  const { data: profile } = useProfile();
  const [loading, setLoading] = useState(false);

  const [formData, setFormData] = useState<Partial<FinanceItem>>({
    type: 'expense',
    title: '',
    amount: 0,
    category: '',
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      // FIX: Cast status as specific literal type
      const newItem = {
        ...
        status: 'approved' as const, // <--- Fixed Type Error
      };

      // 1. Save to Cloud
      const { error } = await supabase.from('finances').insert(newItem);
      if (error) throw error;

      // 2. Save to Local
      await db.finances.add({ ...newItem, synced: 1 });

      toast.success("Saved to Cloud Ledger");
      onComplete();
      onClose();
      setFormData(prev => ({ ...prev, amount: 0, title: '', description: '', category: '' }));
    } catch (err) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Update Unit Ledger">
      <div className="space-y-4">
        ...
        <div>
          ...
          <input type="number" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-lg font-bold text-slate-900 outline-none focus:border-blue-500" value={formData.amount || ''} onChange={e => setFormData({ ...formData, amount: parseFloat(e.target.value) })} />
        </div>
        <div>
...
// --- MODAL: NEW BUDGET REQUEST (SYNCED) ---
const AddRequestModal = ({ isOpen, onClose, onComplete }: { isOpen: boolean; onClose: () => void; onComplete: () => void }) => {
  const { data: profile } = useProfile();
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<Partial<BudgetRequest>>({
    title: '', amount: 0, description: '', request_date: new Date().toISOString().split('T')[0]
  });

  const handleSubmit = async () => {
    if (!profile?.unit_id || !formData.amount || !formData.title) return;
    setLoading(true);
    try {
      // FIX: Cast status as specific literal type
      const newRequest = {
        ...
        status: 'pending' as const, // <--- Fixed Type Error
        admin_comment: '',
      };

      // 1. Save to Cloud
      const { error } = await supabase.from('budget_requests').insert(newRequest);
      if (error) throw error;

      // 2. Save to Local
      await db.requests.add({ ...newRequest, synced: 1 });

      toast.success("Request sent to Admin");
      onComplete();
      onClose();
      setFormData({ title: '', amount: 0, description: '', request_date: new Date().toISOString().split('T')[0] });
    } catch (err) {
      console.error(err);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="New Budget Request">
      <div className="space-y-4">
        <div><label className="text-xs font-bold text-slate-500 uppercase">Title</label><input className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-500" placeholder="e.g. Purchase of HDMI Cables" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} /></div>
        <div><label className="text-xs font-bold text-slate-500 uppercase">Estimated Cost (₦)</label><input type="number" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-lg font-bold text-slate-900 outline-none focus:border-blue-500" value={formData.amount || ''} onChange={e => setFormData({ ...formData, amount: parseFloat(e.target.value) })} /></div>
        <div><label className="text-xs font-bold text-slate-500 uppercase">Details</label><textarea className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-500" rows={3} placeholder="Why do we need this?" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} /></div>
        <div className="flex justify-end gap-2 pt-2"><Button variant="ghost" onClick={onClose}>Cancel</Button><Button onClick={handleSubmit} isLoading={loading}>Send Request</Button></div>
      </div>
    </Modal>
  );
};
...
export const FinancePage = () => {
  ...
  const ledgerItems = useLiveQuery(() => db.finances.orderBy('date').reverse().toArray(), []);
  const requests = useLiveQuery(() => db.requests.orderBy('request_date').reverse().toArray(), []);
  ...
  useEffect(() => {
    const syncFinance = async () => {
      ...
      // Fetch Ledger
      const { data: ledgerData } = await supabase.from('finances').select('*').eq('unit_id', profile.unit_id);
      if (ledgerData) {
        await db.finances.clear();
        await db.finances.bulkPut(ledgerData.map((i: any) => ({ ...i, synced: 1 })));
      }

      // Fetch Requests
      const { data: reqData } = await supabase.from('budget_requests').select('*').eq('unit_id', profile.unit_id);
      if (reqData) {
        await db.requests.clear();
        await db.requests.bulkPut(reqData.map((i: any) => ({ ...i, synced: 1 })));
      }
    };
    syncFinance();
  }, [profile?.unit_id]);
  ...
  // Stats
  const totalIncome = ledgerItems?.filter(t => t.type === 'income').reduce((a, c) => a + c.amount, 0) || 0;
  const totalExpense = ledgerItems?.filter(t => t.type === 'expense').reduce((a, c) => a + c.amount, 0) || 0;
  ...
  return (
    <div className="space-y-6 pb-20">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredRequests.length === 0 ? ( <tr><td colSpan={6} className="px-6 py-8 text-center text-slate-500">No requests sent yet.</td></tr> ) : ( filteredRequests.map((req, index) => (
                    <tr key={req.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 font-semibold text-slate-900 border-r border-slate-100">{req.title}</td>
                      ...
                      <td className="px-4 py-3 text-right text-slate-500 text-xs">{formatDate(req.request_date)}</td>
                    </tr>
                  )))}
              </tbody>
            </table>
      ...
      <AddLedgerModal isOpen={isLedgerModalOpen} onClose={() => setIsLedgerModalOpen(false)} onComplete={() => {}} />
      <AddRequestModal isOpen={isRequestModalOpen} onClose={() => setIsRequestModalOpen(false)} onComplete={() => {}} />
    </div>
  );
};