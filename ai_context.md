...
Last Updated: Jan 2026 (Admin Portal & Online Migration Complete)
...
- **Security Strategy**: Tiered RLS (Row Level Security) with "Circuit Breaker" function.
...
- **Dashboard**: `src/pages/admin/AdminDashboard.tsx` (Global Stats: Inventory & Pending Requests only).

### **B. Unit Dashboard (Core Modules)**
...
5.  **Souls (Evangelism)**: `src/pages/dashboard/Souls.tsx`
    -   *Modal*: `AddSoulReportModal.tsx`
    -   *Table*: `soul_reports` (Row-based recording).
...
-   **Recursion Prevention**: We use a `SECURITY DEFINER` function `is_admin()` in RLS policies to prevent infinite loops when admins query tables that link back to profiles.
...
## 5. Deployment Notes
-   **Authentication**: Custom `Login.tsx` redirects users based on `profile.role`.
-   **Navigation**: `/admin` is strictly separated from `/dashboard`.